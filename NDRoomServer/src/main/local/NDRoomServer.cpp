...
initialiseSingleton(NDRoomServer);

NDRefCounted<NDServerManager>	refNDServerManager( new NDServerManager );
NDRefCounted<NDRoomServer>		refNDRoomServer( new NDRoomServer );
...
int main(int argc, char** argv)
{
	...
	ND_EXCEPTION_PROTECTED_CATCH( sNDRoomServer.getDumpFilePath(), exceptionDispose )
...
int mainWapper( int argc, char** argv )
{
	...
	if ( NDFalse == sNDRoomServer.initialize( ROOM_SERVER, nServerID, "ServerConfig.xml") )
	{
		NDLOG_ERROR( " NDRoomServer initialize failed! " )
		return 1;
	}

	//start server service;
	if ( NDFalse == NDServerManager::getSingleton().startServer( NDRoomServer::getSingletonPtr() ) )
	{
		NDLOG_ERROR( " NDRoomServer startServer failed! " )
		return 1;
	}

	//set server started state;
	sNDRoomServer.setState( NDLocalServer::eServState_Started );
...
void exceptionDispose( void *pParam )
{
	...
	sNDRoomServer.setState( NDLocalServer::eServState_Stopping );
	sNDRoomServer.logMgr()->flush();
}
...
BOOL WINAPI handlerRoutine( DWORD dwCtrlType )
{
	NDRoomServer* pRoomServer		= NDRoomServer::getSingletonPtr();
	NDServerManager* pServerMgr		= NDServerManager::getSingletonPtr();
	if ( NULL == pRoomServer || NULL == pServerMgr )
	{
		return FALSE;
	}

	pRoomServer->setState( NDLocalServer::eServState_Stopping );
	...
	pRoomServer->logMgr()->close();

	SAFE_DELETE( pServerMgr )
	SAFE_DELETE( pRoomServer )
